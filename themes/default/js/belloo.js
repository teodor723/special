...
function checkFUR(){
	...
	if(plugins['fakeUsersInteractions']['enabled'] == 'Yes'){
		...
		var check = Math.floor(Math.random()*(100-1+1)+1);
		if(plugins['fakeUsersInteractions']['send_auto_messages'] == 'Yes' && plugins['fakeUsersInteractions']['auto_message_change'] >= check && profile_info.status == "y" && profile_info.fake == 1 && profile_info.id != user_info.id){
        	sendAutoMsg(profile_info.id,user_info.id);
        }
	}	
}
...
function checkAutoMsg(id,r,s){
	...
	$.getJSON( aUrl, {action: 'check_rnd_msg', query: query} ,function( data ) {
		if(data.result == 'OK'){
			...
			if(profile_info.status == "y" && profile_info.fake == 1 && profile_info.id != user_info.id) {
			    sendAutoMsg(profile_info.id,user_info.id);
			}
// 			console.log('msg sent');
			...
			if(failCount < 3){
			    if(profile_info.status == "y" && profile_info.fake == 1 && profile_info.id != user_info.id) {
				    sendAutoMsg(profile_info.id,r,'Yes');	
			    }
			} else {
...
switch (url) {
	...
	case "discover":
		storyPage = 'discover';
		var loading=theme_source()+"/images/loading.svg";
	    $("#dis_photos").html('<img src="'+loading+'" style="margin:10% auto;display:block;" >');
		profilePhoto();
		updateDiscoverResults();
		discoverStoriesPreview();
// 		setTimeout(function(){
...
// 		},800);
	...
	case 'meet':
	    var loading=theme_source()+"/images/loading.svg";
	    $("#meet_section").html('<img src="'+loading+'" style="margin-top:40px;" >');
	...
	$('[data-murl]').click(function(e){
		...
		switch (murl) {
			...
			case "meet":	
				$('#data-content').css("opacity","0.5");
				var loading=theme_source()+"/images/loading.svg";
	            $("#data-content").html('<img src="'+loading+'" style="text-align: center;margin: 16% auto;display: block;vertical-align: middle;" >');
				checkScrollBars('scroll');
				profile_info = '';
				$.ajax({
					url: request_source()+'/user.php', 
...
$("#upload-area").dmUploader({
	...
	onUploadSuccess: function(id, file){
	 if('contetnerror' in file) {
		 $('[data-manage-media='+id+']').remove();
	     swal({title: site_lang[1037]['text'],   text: site_lang[1036]['text'], type: "error" }, function(){ });
	     return false;
	 }
	 
	 if('chatcontetnerror' in file) {
		 $("#me:last-child").remove();
	     swal({title: site_lang[1037]['text'],   text: file.chatcontetnerror, type: "error" }, function(){ });
	     return false;
	 }
