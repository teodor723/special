...
function getFakeUsersChat($tab) {
	...
	if($tab == 'New'){
		$query = $mysqli->query("SELECT DISTINCT id,s_id,r_id FROM chat where fake = 1  and seen = 0 order by id DESC");
	} else {
		$query = $mysqli->query("SELECT DISTINCT id,s_id,r_id FROM chat where fake = 1  order by id DESC");
	}
