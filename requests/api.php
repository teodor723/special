...
// Always use session user ID, never from GET/POST parameters
...
// 			if(emaillistverify_fn('enable_emaillistverify')=="Yes"){
// 			    $e_api_key=emaillistverify_fn('emaillistverify_api_key');
// 			    $key = $e_api_key;
...
// 			}
			...
			if($email_check->num_rows == 1 ){
				...
				$query = "INSERT INTO users (name,email,pass,age,birthday,gender,city,country,lat,lng,looking,lang,join_date,bio,s_gender,s_age,credits,online_day,ip,last_access,username,join_date_time,app_id,referral,country_code) VALUES ('".$name."', '".$email."','".$pswd."','".$age."','".$birthday."','".$gender."','".$city."','".$country."','".$lat."','".$lng."','".$looking."','".$lang."','".$date."','".$bio."','".$looking."','18,60,1',0,0,'".$ip."','".time()."','".$username."','".time()."','".$dID."','".$ref."','".$country_code."')";	
				if ($mysqli->query($query) === TRUE) {
					$last_id = $mysqli->insert_id;
					$mysqli->query("INSERT INTO users_videocall (u_id,peer_id) VALUES ('".$last_id."',0)");
					$amount=120;
					$query = "UPDATE users SET credits = credits+'".$amount."' WHERE id= '".$last_id."'";
					$mysqli->query($query);
					$time = time();
					$reason="Reward credits for register";
					$reward="newAccountFreeCredit";
					$mysqli->query("INSERT INTO users_rewards (uid,reward,reward_type,reward_date,reward_amount) VALUES ('".$last_id."','".$reward."','credits','".$time."','".$amount."')");

					$mysqli->query("INSERT INTO users_credits (uid,credits,reason,time,type) VALUES ('".$last_id."','".$amount."','".$reason."','".$time."','added')");	
					...
					$query = "UPDATE users SET s_age = '18,60,1' WHERE id= '".$last_id."'";
					$mysqli->query($query);
		...
		if ($check->num_rows == 1){	
			...
			$query = "INSERT INTO users (name,email,pass,age,gender,city,country,lat,lng,looking,lang,join_date,s_gender,s_age,verified,facebook_id,credits,app_id)
									VALUES ('".$name."', '".$email."','".crypt($pswd)."','20','".$gender."','".$city."','".$country."','".$lat."','".$lng."','".$looking."','".$_SESSION['lang']."','".$date."','".$looking."','18,60,1',1,'".$fuid."','".$sm['config']['free_credits']."','".$dID."')";	
			if ($mysqli->query($query) === TRUE) {
				$last_id = $mysqli->insert_id;
				$_SESSION['user'] = $last_id;
				setcookie("user", $last_id, 2147483647);	
		...
		case 'unreadMessageCnt':
			$id = secureEncode($_GET['dID']);
			$arr = array();			
			$arr['unreadMessageCount'] = checkUnreadMessages($id);
			echo json_encode($arr);		
		break;
		...
		case 'config':
			...
			if($device_check->num_rows == 0 ){
				...
				$arr['user']['notification'] = userNotifications($pass->id);
				$arr['config']['randomfakeOnline'] = getRandomFakeOnline('id',$sm['user']['looking']);
			}
			$sm['config']['logo']=($_SERVER['SERVER_NAME']=='special-dating.com')?$sm['metasettings']['website_desktop_logo']:getvaluemetax('_domain_desktop_logo',$_SERVER['SERVER_NAME']);
			$sm['config']['title']=$sm['metasettings']['website_title'];
			$sm['config']['description']=$sm['metasettings']['description'];
			$sm['config']['keywords']=$sm['metasettings']['keywords'];
			...
			$arr['config']['theme']['logo']['val']=($_SERVER['SERVER_NAME']=='special-dating.com')?$sm['metasettings']['website_mobile_logo']:getvaluemetax('_domain_mobile_logo',$_SERVER['SERVER_NAME']);
			$arr['config']['theme']['profileOff_logo']['val']=($_SERVER['SERVER_NAME']=='special-dating.com')?$sm['metasettings']['website_mobile_logo']:getvaluemetax('_domain_mobile_logo',$_SERVER['SERVER_NAME']);
			$arr['app']->logo=($_SERVER['SERVER_NAME']=='special-dating.com')?$sm['metasettings']['website_mobile_logo']:getvaluemetax('_domain_mobile_logo',$_SERVER['SERVER_NAME']);
			$arr['config']['theme']['favicon']['val']=($_SERVER['SERVER_NAME']=='special-dating.com')?$sm['metasettings']['website_favicon']:getvaluemetax('_domain_favicon',$_SERVER['SERVER_NAME']);
		...
		case 'spotlight':
			...
			$time_now = time()-300;
			$i = 0;
			$lat = isset($sm['user']['lat'])?$sm['user']['lat']:0;
			$lng = isset($sm['user']['lng'])?$sm['user']['lng']:0;

			$limit = $sm['plugins']['spotlight']['limit'];
			$area = $sm['plugins']['spotlight']['area'];
			$autoWorldwide = $sm['plugins']['spotlight']['worldwide'];
			$only_opposite = $sm['plugins']['spotlight']['opposite'];
				...
				if ($spotlight->num_rows > 0) {
					while($spotl = $spotlight->fetch_object()){
						...
						$info['spotlight'][] = array(
							  ...
							  "total_photos" => count(userAppPhotos($spotl->u_id)),
							  "error" => 0,
					...
					if($spotlight && $spotlight->num_rows > 0){ 
						while($spotl = $spotlight->fetch_object()){	
							...
							$info['spotlight'][] = array(
								  ...
								  "total_photos" => count(userAppPhotos($spotl->id)),
								  "error" => 0,
		...
		case 'updateCredits':
			...
			$time = time();
			if($type == 1){
				$query = "UPDATE users SET credits = if(credits-'".$amount."'>0,credits-'".$amount."',0) WHERE id= '".$uid."'";
				$mysqli->query($query);
			} else {
			...
			if($type == 'reward'){
				...
				$mysqli->query("INSERT INTO users_rewards (uid,reward,reward_type,reward_date,reward_amount) VALUES ('".$uid."','".$reward."','credits','".$time."','".$amount."')");

				//$mysqli->query("INSERT INTO users_credits (uid,credits,reason,time,type) VALUES ('".$uid."','".$amount."','".$reason."','".$time."','added')");			
			}
		...
		case 'meet':
			...
			if($check < $check2 && $sm['plugins']['fakeUsersGenerator']['enabled'] == 'Yes' && $apiLimit == 'No'){
				...
				// 					(id,u_id,photo,thumb,profile,private,blocked,fake) VALUES (".$p->id.",'".$val->id."', '".$p->photo."', '".$p->thumb."',".$i.",".$p->private.",".$p->private.",1)");
				// 			} 							
				// 		} else {
...
//                 AND id <> '".$sm['user']['id']."' ".(($status_filter)?$status_filter:'')."  ".(($sm['user']['country'])?$country_filter:'')." HAVING distance < $radius ORDER BY RAND() , fake ASC";
//                 $query2 = "SELECT id, ( 6371 * acos( cos( radians($lat) ) * cos( radians( lat ) )
//                 * cos( radians( lng ) - radians($lng) ) + sin( radians($lat) ) * sin( radians( lat ) ) ) ) AS distance 
//                 FROM users
//                 WHERE age BETWEEN '".$age1."' AND '".$age2."'
//                 AND id <> '".$sm['user']['id']."' ".(($status_filter)?$status_filter:'')."  ".(($sm['user']['country'])?$country_filter:'')." HAVING distance < $radius ORDER BY RAND()";           
//             } else {
//                 $query = "SELECT id, ( 6371 * acos( cos( radians($lat) ) * cos( radians( lat ) )
//                 * cos( radians( lng ) - radians($lng) ) + sin( radians($lat) ) * sin( radians( lat ) ) ) ) AS distance 
//                 FROM users WHERE gender = '".$looking."' AND age BETWEEN '".$age1."' AND '".$age2."' AND id <> '".$sm['user']['id']."' ".(($status_filter)?$status_filter:'')." ".(($sm['user']['country'])?$country_filter:'')." HAVING distance < $radius ORDER BY RAND() , fake ASC";
//                 $query2 = "SELECT id, ( 6371 * acos( cos( radians($lat) ) * cos( radians( lat ) )
//                 * cos( radians( lng ) - radians($lng) ) + sin( radians($lat) ) * sin( radians( lat ) ) ) ) AS distance 
//                 FROM users WHERE gender = '".$looking."' AND age BETWEEN '".$age1."' AND '".$age2."' AND id <> '".$sm['user']['id']."' ".(($status_filter)?$status_filter:'')." ".(($sm['user']['country'])?$country_filter:'')." HAVING distance < $radius ORDER BY RAND() , fake ASC";
//             }
...
//             $sm['meet_result'] = $result2->num_rows;
			...
			$user_id = $sm['user']['id'];
			$country_filter_sql = (!empty($sm['user']['country']) && !empty($country_filter)) ? $country_filter : '';
			$status_filter_sql = (!empty($status_filter)) ? $status_filter : '';
			...
			// Save count
			$sm['meet_result'] = $result2 ? $result2->num_rows : 0;
			...
			if ($result->num_rows > 0) {
				while($row = $result->fetch_object()){
					...
					$info['result'][] = array(
						  ...
						  "total_photos" => count(userAppPhotos($id)),
						  "margin" => $margin,
						  "story" => "0",
					  	  "stories" => [],						  
			...
			if ($query->num_rows > 0) {
				while($row = $query->fetch_object()){
					...
					$storiesFilter = 'where uid = '.$sm['search']['id'].' and storyTime >'.$storyFrom.' and deleted = 0 and visible = 1';	
					...
					$info['popular'][] = array(
					  ...
					  "total_photos" => count(userAppPhotos($sm['search']['id'])),
					  "story" => "0",
				  	  "stories" => [],						  
		...
		case 'dateApply':
			$id = secureEncode($_GET['id']);
			$cols = 'date_id,user_id';
			$vals = $id.','.$sm['user']['id'];
			insertData('dating_requests',$cols,$vals);			
			echo 'OK';
		break;

		case 'dateRequests':
			$filter = secureEncode($_GET['filter']);
			$data = secureEncode($_GET['data']);
			if(isset($_GET['mobile'])){
				...
				$data['results'] = $i;
				$data['data'] = [];
				$query = $mysqli->query("SELECT * FROM dating WHERE dating_time > ".$time." AND dating_gender = ".$sm['user']['gender']." || uid = ".$sm['user']['id']. " ORDER BY dating_time ASC");			
				if ($query->num_rows > 0) {
					while($row = $query->fetch_object()){
						$i++;
						$photo = profilePhoto($row->uid,1);
						$row->photo = $photo;
						$row->name = getData('users','name','where id ='.$row->uid);
						$row->city = $row->dating_city;
						$row->age = getData('users','age','where id ='.$row->uid);
						$row->applyRequest = getData('dating_requests','date_id','where date_id ='.$row->id.' AND user_id ='.$sm['user']['id']);
		...
		case 'getVisitors':
			...
			$arr['visitors'] = null;
			$query = $mysqli->query("
				SELECT uv.u2, uv.timeago, uv.notification,
					   u.name, u.username, u.age, u.city, u.last_access, u.premium, u.credits
				FROM users_visits uv
				INNER JOIN users u ON u.id = uv.u2
				WHERE uv.u1 = '".$id."' AND uv.u2 <> '".$id."'
				ORDER BY uv.timeago DESC
			");
			if ($query->num_rows > 0) { 
				while($result = $query->fetch_object()){
					$visitorId = $result->u2;
					$firstNameParts = explode(' ',trim($result->name));
					$firstNameParts = explode('_',trim($firstNameParts[0]));
					$firstName = $firstNameParts[0];
					$displayName = $result->name;

					if(isset($sm['plugins']['settings']['onlyUsername']) && $sm['plugins']['settings']['onlyUsername'] == 'Yes'){
					...
					$fan = isFan($visitorId,$id);
					$match = ($fan == 1 && isFan($id,$visitorId) == 1) ? 1 : 0;
					$lastMessageDiff = get_time_difference_php($result->timeago);
					...
					$arr['visitors'][] = array(
						  ...
						  "age" => $result->age,
						  "city" => $result->city,
						  "last_a" => $result->last_access,
						  "premium" => $result->premium,							  
						  "photo" => profilePhoto($visitorId),
						  "fan" => $fan,
						  "match" => $match,					  
						  "error" => 0,
						  "story" => '0',
					  	  "stories" => [],						  
						  "status" => userFilterStatus($visitorId),
						  "last_m" => $sm['alang'][127]['text'].' '.$lastMessageDiff.' ago',
						  "last_m_time" => $lastMessageDiff,
		...
		$query2 = $mysqli->query("SELECT id,s_id,r_id,seen,notification,gif,gift,time,translated,translated_text FROM chat WHERE r_id = '".$id."' || s_id = '".$id."' order by id desc");
		if ($query2->num_rows > 0) { 
			while($result2 = $query2->fetch_object()){
				if (!in_array($result2->s_id, $arr)){
					$arr[] = $result2->s_id;			  
					$filterId = $result2->s_id;
					$cols = 'id,name,username,city,country,age,last_access,premium,credits,lang';
					$filter = 'WHERE id = '.$filterId;
					$userData = getSelectedArray($cols,'users',$filter,'ID DESC','');
					$sm['profile'] = $userData[0];
					$first_name = explode(' ',trim($sm['profile']['name']));
					...
					if($result2->r_id == $id && $result2->seen == 2){			
						...
						if (strpos($last_m , 'DATE-REQUEST-') !== false) {
							$last_m = "Applied your date request";
						}

						if($result2->translated == 1 && $result2->s_id == $id){
							$last_m = $result2->translated_text;
						}
						...
						$arr['matches'][] = array(
							  ...
							  "credits" => $sm['profile']['credits'],
							  "lang_prefix" => getData('languages','prefix','WHERE id = '.$sm['profile']['lang']),
...
// 			}
			...
			if(emaillistverify_fn('enable_emaillistverify')=="Yes"){
                ...
                if($response!="ok"){
                    $arr['validEmail'] = 'No';	
				    $arr['validEmailMsg'] = $sm['lang'][1035]['text'];
                }
			}
...
// 			getUserInfo($uid);
		...
		case 'sendMessage':
			...
			if($type == 'credits'){
				$message = '<b>'.$sm['lang'][583]['text'].' '.$data[4].' '.$sm['lang'][128]['text'].'!</b><img src="'.$sm['config']['theme_url'].'/images/icon-coins.png" style="width:34px;">';
				...
				$message = $sm['lang'][583]['text'].' '.$data[4].' '.$sm['lang'][128]['text'].'!';
			...
			if($type == 'text' || $type == 'videocall'){
				if(isset($data[4])){
					if($data[4] == 'translated'){
						$mysqli->query("INSERT INTO chat (s_id,r_id,time,message,fake,online_day,translated,translated_text) VALUES ('".$s_id."','".$r_id."','".$time."','".$message."','".$fake."','".$online_day."',1,'".$data[5]."')");
					} else {
		...
		case 'userChat':
			...
			if ($spotlight->num_rows > 0) { 
				while($spotl = $spotlight->fetch_object()){					
					...
					if (strpos($message, 'DATE-REQUEST-') !== false) {
						$dateData = explode('-',$message);
						$dateId = $dateData[2];
						$dateMsg = getData('dating','dating_description','WHERE id = '.$dateId);
						$message = 'Applied your date request 
						<br><small>"'.$dateMsg.'"</small>';
					}
		...
		case 'phoneVerificationModal':
			...
			$photo = "'".$sm['user']['profile_photo']."'";
			$return = '
				<div id="verify-phone-modal" class="hp-modal-wrp coin-modal" style="display:none;">
				    <div class="hpm-body">
				        <div class="hpm-title" style="font-size: 1.8rem;">
				            <div class="brick brick--lg box-shadow">
				                <div class="brick-img profile-photo" style="background-image:url('.$photo.');height:100%;border-radius:50%"></div>
				            </div>             
				            <h4 style="padding-top:15px">'.$sm['user']['username'].', '.$sm['lang'][982]['text'].'</h4>
				        </div>
				        <div class="hpm-cont">

				            <div class="if-title vivify fadeIn delay-100" id="verificationNumber" style="padding-bottom:60px">
				                <p style="margin-top:20px">'.$sm['lang'][983]['text'].'</p>
				                <div class="ift-code" style="display:block">

				                    <input class="rlt-referral-code box-shadow" style="margin-top:20px" onkeyup="checkValidPhone()" id="currentUserPhoneNumber" type="number">


				                    <input class="ift-copy-code box-shadow hidden" style="margin-top:20px;width:180px!important;" maxlength="4" id="userCodeSMS" onchange="verifySMSCode()" onkeyup="verifySMSCode()" placeholder="CODE">  

				                    <span class="ift-copy-code box-shadow gradient13 uploadingGray" id="send-sms-verification" onclick="" style="margin-top:20px">
				                        '.$sm['lang'][984]['text'].'
				                    </span>
				                </div>  
				            </div>
				            <div style="position: relative;text-align: center;font-size: 0.8rem;">
				                <a href="javascript:;" id="reSendSmsBtn" onclick="" style="color:#333!important">'.$sm['lang'][985]['text'].'</a>
				            </div>
                               
				            <div class="hpm-close close-icon" onclick="phoneVerificationModal(`close`)" style="margin-right:10px"></div>
				        </div>
				    </div>
				    <div class="hpm-overlay" onclick="phoneVerificationModal(`close`)"></div>
				</div>';
		...
		case 'game':
			...
			if($gender == $all){
				$u_total = $mysqli->query("SELECT id, ( 6371 * acos( cos( radians('".$sm['user']['lat']."') ) * cos( radians( lat ) ) * 
						  ...
						  FROM users
						  WHERE age BETWEEN '".$age1."' AND '".$age2."'				  
						  ORDER BY distance ASC, last_access DESC LIMIT 0,50");
			}else{
				$u_total = $mysqli->query("SELECT id, ( 6371 * acos( cos( radians('".$sm['user']['lat']."') ) * cos( radians( lat ) ) * 
						  cos( radians( lng ) - radians('".$sm['user']['lng']."') ) + sin( radians('".$sm['user']['lat']."') ) * sin(radians(lat)) ) )
						  AS distance 
			...
			if(count($resultado) == 0){
				...
				foreach($resultado as $user_g){
					...
					$extra = 86400 * $storyFrom;
					$storyFrom = $time - $extra;
					$storiesFilter = 'where uid = '.$sexy_game->id.' and storyTime > '.$storyFrom.' and deleted = 0 and visible = 1';						
					$info['game'][] = array(
						  "id" => $sexy_game->id,
						  "name" => $sm['profile']['name'],
	...
	break;
		...
		case 'viewStories':
			...
			$lng = $data[1];
			$looking = $data[2];
			$offset = isset($data[3])?$data[3]:0;

			$stories = discoverStoriesMobile($lat,$lng,$looking,$offset);
			echo $stories;     	  
		break;	
		...
		case 'finishMingle':
			if(isset($_GET['uid'])){
				...
				$checkUser = getData('users','credits','WHERE id = '.$me);
				if($checkUser != 'noData'){
		...
		case 'sendSMSCode':
			...
			$smskey = generateKey(4);

			updateData('users','sms_verification',$smskey,'WHERE id = '.$uid);
			updateData('users','telephone',$number,'WHERE id = '.$uid);

			$smsMsg = '['.$sm['config']['name'].'] - '.$sm['user']['username'].' '.$sm['lang'][988]['text'].' '.$smskey;

			sendSMS($sm['plugins']['verification']['phone_carrier'],$number,$smsMsg);
			$arr = [];
			$arr['newcode'] = $smskey;
			$arr['newphone'] = $number;
		...
		case 'phoneVerified':
			$uid = secureEncode($_GET['query']);
			updateData('users','sms_verified',1,'WHERE id = '.$uid);			
		break;
		...
		case 'updateCountryCode':
			...
			$uid = $data[0];
			$code = $data[1];
			updateData('users','country_code','"'.$code.'"','WHERE id = '.$uid);
		break;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	switch (secureEncode($_POST['action'])) {
		...
		case 'createDateRequest':
			$arr = array();
			$user = secureEncode($_POST['user']);
			$description = secureEncode($_POST['description']);
			$gender = secureEncode($_POST['gender']);
			$place = secureEncode($_POST['place']);
			$when1 = secureEncode($_POST['when1']);
			$when2 = secureEncode($_POST['when2']);						
			$budget = secureEncode($_POST['budget']);
			...
			//$cols = 'uid,dating_description,dating_gender,dating_place,dating_when,dating_time,dating_budget,dating_city,dating_type';
			//$vals = $user.',"'.$description.'","'.$gender.'","'.$place.'","'.$when.'","'.$time.'","'.$budget.'","'.$city.'","'.$type.'"';
			$cols = 'uid,dating_description,dating_place,dating_when,dating_time,dating_budget,dating_city,dating_type';
			$vals = $user.',"'.$description.'","'.$place.'","'.$when.'","'.$time.'","'.$budget.'","'.$city.'","'.$type.'"';			
			insertData('dating',$cols,$vals);

			$arr['OK'] = 'Yes';
			echo json_encode($arr);		
		break;

		case 'deleteDateRequest':
			$arr = array();
			if($uid > 0){
				if(isset($_POST['id'])){
					$delete = 'WHERE id = '.secureEncode($_POST['id']);
				} else {
					$delete = 'WHERE uid = '.$uid;	
				}
				deleteData('dating',$delete);
			}
			$arr['OK'] = 'Yes';
			echo json_encode($arr);		
		break;
		...
		case 'upload_feed':
			...
			$time = time();
			$user = secureEncode($_POST['user']);
			$src = secureEncode($_POST['src']);
			$meta = secureEncode($_POST['meta']);
			$checkSRC = substr($src, -1);
			if($checkSRC == ','){
				$src = substr($src, 0, -1);
			}

	        $cols = 'uid,post_type,post_price,post_src,post_meta,post_premium,post_blur,likes,time,visible';
	        $vals = $user.',"image",0,"'.$src.'","'.$meta.'","No",
	        "'.$src.'",0,'.$time.',1';
			...
			if($story == 1){
				...
				if($sm['plugins']['story']['reviewStory'] == 'No'){
          		...
          		$query = "INSERT INTO users_story (uid,storyTime,story,storyType,lat,lng,visible)
          			 VALUES ('".$uid."','".$time."','".$arr['data']['photo']."','".$type."','".$lat."','".$lng."',".$visible.")";
          		if ($mysqli->query($query) === TRUE) {
          			$last_id = $mysqli->insert_id;
		...
		case 'addReward':
			...
			if($type == 'credits'){
				updateData('users','credits','credits+'.$amount,'WHERE id = '.$user);
			} else {
				$time = time();	
				$extra = 86400 * $amount;
				$premium = $time + $extra;				
				updateData('users_premium','premium',$premium,'WHERE uid = '.$user);
			}
		...
		case 'sendPaymentProof':
			$arr = array();
			$user = secureEncode($_POST['user']);
			$gateway = secureEncode($_POST['gateway']);
			$type = secureEncode($_POST['type']);
			$package = secureEncode($_POST['package']);
			$title = secureEncode($_POST['title']);
			$price = secureEncode($_POST['price']);
			$proof = secureEncode($_POST['proof']);
			$proof_image = secureEncode($_POST['proof_image']);
			$time = time();															

			$order_id = uniqid();

			$cols = 'order_id,user_id,order_type,order_package,order_gateway,order_status,order_date,order_proof,order_proof_img,order_title,order_custom';
			$vals = '"'.$order_id.'",'.$user.',"'.$type.'",'.$package.',"'.$gateway.'","pending",'.$time.',"'.$proof.'","'.$proof_image.'","'.$title.'",1';
			insertData('orders',$cols,$vals);

			$arr['order'] = $order_id;

			$to = $sm['config']['email'];
			$subject = $gateway." order for review ".$order_id;
			$txt = "Congratulations!, there is a new order proof from ".$gateway." for ".$title." go to your admin panel and review the proof from there";
