...
// Require admin authentication for all admin operations
requireAdmin();
...
switch ($_POST['action']) {
	...
	case 'save_multi_domain_settings':
		$key_name = $_POST['key_name'];
		$key_value = $_POST['key_value'];
		$arr = array();
		if($key_name=="robots_text") {
		    $myfile = fopen(__DIR__."/../robots.txt", "w") or $arr['errmsg']=("Unable to open robots.txt file!");
            $txt = $key_value;
            fwrite($myfile, $txt);
            fclose($myfile);
		}
		if (isset($_POST['filename']) && isset($_POST['filetype'])) {
		    $fileData = $_POST['key_value'];
            ...
            if (in_array($fileExtension, $allowedfileExtensions)) {
                $uploadFileDir = __DIR__.'/../medias/';
                $dest_path = $uploadFileDir . $newFileName;
                if (file_put_contents($dest_path, $decodedData)) {
                    $key_value="https://" . $_SERVER['SERVER_NAME']."/medias/".$newFileName;
                    $arr['msg']='File is successfully uploaded.';
                } else {
                    $arr['errmsg']='There was an error saving the file.';
                }
            } else {
                $arr['errmsg']='Upload failed. Allowed file types: ' . implode(',', $allowedfileExtensions);
            }
		}
		$key_check = $mysqli->query("SELECT * FROM multi_settings WHERE config_key = '".$key_name."'");
		if($key_check->num_rows == 0 ){
			$mysqli->query('INSERT INTO multi_settings (config_key,config_value) VALUES ("'.$key_name.'","'.$key_value.'")');
		} else {
		    $mysqli->query("UPDATE multi_settings SET config_value = '".$key_value."' WHERE config_key = '".$key_name."'");
		}
		if(isset($_POST['n']) && !empty($_POST['n'])) {
		    $n=$_POST['n'];
		    $mysqli->query("UPDATE multi_settings SET config_value = '".$n."' WHERE config_key = 'total_domains'");
		}
		echo json_encode($arr);
	break;
	...
	case 'cp_status':
		...
		$mysqli->query("UPDATE custom_payments SET p_status = ".$status." WHERE id = ".$id);
		$arr = array();
		echo json_encode($arr);
	break;	
	...
	case 'cp_delete':
		$id = $_POST['p_id'];
		$mysqli->query("DELETE FROM custom_payments WHERE id = ".$id);
		$arr = array();
		echo json_encode($arr);
	break;
	...
	$search = getArray('custom_payments',$filter,$orderby.' DESC','');
    $i=0;
    ...
    if(isset($search)){
      foreach ($search as $value) { 
      	...
      	$package = 'All';
      	if($value['p_package'] == 0){
      		$type = 'Cheapest';
      	}
      	if($value['p_package'] == 1){
      		$type = 'Middle';
      	} 
      	if($value['p_package'] == 2){
      		$type = 'Expensive';
      	}       	     	

    	$logo = '
    		<img style="max-width:120px;height:auto;" src="'.$value['p_logo'].'" class="avatar-img square">';
		...
		$dropdown.= '			  
			<a class="dropdown-item" href="javascript:;" 
			style="font-size: 13px;'.$adStatusDropdownInactive.'" 
			onclick="customPaymentStatus('.$value['id'].',1)">
				Show Payment
			</a>
			<a class="dropdown-item" href="javascript:;" 
			style="font-size: 13px;'.$adStatusDropdownActive.'" 
			onclick="customPaymentStatus('.$value['id'].',0)">
				Hide Payment
			</a>							  
			<div class="dropdown-divider"></div>
			<a class="dropdown-item" href="javascript:;" style="font-size: 13px" 
			onclick="deleteCP('.$value['id'].')">
				Delete Payment
			</a>';        
        ...
        $data[$i]='
            <tr class="data-search-verifications">
                ...
                <td>
                	<small>'.$value['p_name'].'</small>
                </td>
                <td>
                	<small class="text-muted">'.$value['p_description'].'</small>
                </td>
                <td>
                	<small class="text-muted">'.$value['p_proof'].'</small>
                </td>                                         
                <td>
                ...
                <td>
                	<small class="text-muted">'.$value['p_external_url'].'</small>
                </td>
                <td>
                	<small class="text-muted">'.$value['p_review_time'].' hs</small>
                </td>                                 
                <td>
	            	<small class="badge badge-'.$adStatusLabel.'">
	            		'.$adStatus.'
	...
	case 'create_custom_payment':
		...
		$cols = substr($cols, 0, -1);
		$values = substr($values, 0, -1);			
		$mysqli->query("INSERT INTO custom_payments ($cols) VALUES ($values)");				
	...
	case 'search_users':
        ...
        $checkReels = getData('plugins','enabled','where name = "reels"');

// 		$search = getSelectedArray('id,name,email,age,city,country,fake,admin,last_access,credits,premium,ip,online_day,verified,moderator,gender','users',$filter,$order.' desc','LIMIT 0,500');
		$query = $mysqli->query("SELECT id,name,email,age,city,country,fake,admin,last_access,credits,premium,ip,online_day,verified,moderator,gender FROM users ".$filter." ORDER BY ".$order);
		...
		$query = $mysqli->query("SELECT id,name,email,age,city,country,fake,admin,last_access,credits,premium,ip,online_day,verified,moderator,gender FROM users ".$filter." ORDER BY ".$order." LIMIT " . $page_first_result . ',' . $results_per_page);
		...
		if(isset($search)){
			foreach ($search as $value) { 
				...
				if($checkReels == 1){
					$addReelDropdown = '
	                  <div class="dropdown-divider"></div>
	                  <a class="dropdown-item" style="font-size: 13px" href="javascript:;" onclick="adminUploadReel('.$value['id'].','.$value['gender'].',`'.profilePhoto($value['id']).'`)">Upload reel</a>
	                  ';
				}
		...
		$arr['data'] = $data;
		$arr['total'] = $total;
		$arr['total_pages'] = $number_of_page;
		echo json_encode($arr);
	break;
	...
	case 'search_post':
	    ...
	    $status = secureEncode($_POST['status']);
	    $posttype = secureEncode($_POST['posttype']);
	    ...
	    if($posttype == 'image'){
	    	$filter = 'WHERE post_type = "image"';
	    	$table = 'feed';	
	    }

	    if($posttype == 'video'){
	    	$filter = 'WHERE post_type = "video"';
	    	$table = 'feed';	
	    }

	    if($posttype == 'audio'){
	    	$filter = 'WHERE post_type = "audio"';
	    	$table = 'feed';	
	    }  

	    if($posttype == 'text'){
	    	$filter = 'WHERE post_type = "text"';
	    	$table = 'feed';	
	    }            
	    
		if($searchInput != ''){
			$filter.=" AND uid = ".$searchInput;
		}

		if($status != 'All'){
			$filter.=' AND visible ='.$status;
		}


		if($ptype != 'All'){
			$filter.=' AND post_premium ="'.$ptype.'"';
		}	  
		...
		$search = getArray($table,$filter,'id DESC','');
	    $i=0;
	    if(isset($search)){
	      foreach ($search as $value) { 
			...
			$arr['searchUserId'] = '';
			if($searchInput != ''){
				$arr['searchUserPhoto'] = profilePhoto($value['uid']);
				$arr['searchUserId'] = $value['uid'];
				$arr['searchUserName'] = getData('users','name','WHERE id = '.$value['uid']);
			}
	      	$checkData = $value['id'];
	      	$id = $value['id'];
	      	$userId = $value['uid'];
	    	...
	    	$adminUpdateDataCustomVal = "'updatePostAdmin'";
	    	$adminUpdateDataVal = '';
			...
			$play_video = "'".$value['post_src']."'";
	      	$uName = getData('users','name','where id ='.$userId);
	      	$uploadedPhotoOnClick = "'".$value['post_src']."',".$userId.",'".$uName."','".$uEmail."','".$uPhoto."'";

	      	$multipleSRC = 'No';
            if(strpos($value['post_src'], ',') !== false){
                $media = explode(',',$value['post_src']);
                $value['post_src'] = $media[0];
                $multipleSRC = 'Yes';
            }
	        ...
	        if($multipleSRC == 'Yes'){
	        	$postType = 'Gallery ('.count($media).')';
	        	$posttypeLabel = 'success';
	        }

	        $postArray = array();
	        
	        $postArray['postType'] = $postType;
	        $postArray['postId'] = $value['id'];
	        $postArray['postUid'] = $value['uid'];
	        $postArray['postSrc'] = $value['post_src'];
	        $postArray['postMeta'] = $value['post_meta'];
	        $postArray['postPremium'] = $value['post_premium'];
			...
			$postArray['action'] = 'updatepost';
			$postArray['method'] = 'postSetPublic';


			$postPublicArrayOnClick = json_encode($postArray,JSON_UNESCAPED_UNICODE);
	        $postPublicArrayOnClick = "'adminUpdateData(".$postPublicArrayOnClick.")'";	        	
	   		$postPublicDropdown.= '<a class="dropdown-item" data-post-dropdown-public="'.$id.'" href="javascript:;" style="font-size: 13px;display:'.$publicDropDown.'" 
		  onclick='.$postPublicArrayOnClick.'> Set Public </a>';
			 


	    	$postArray['method'] = 'postSetPrivate';


			$postPublicArrayOnClick = json_encode($postArray,JSON_UNESCAPED_UNICODE);
	        $postPublicArrayOnClick = "'adminUpdateData(".$postPublicArrayOnClick.")'";	        	
	   		$postPublicDropdown.= '<a class="dropdown-item" data-post-dropdown-private="'.$id.'" href="javascript:;" style="font-size: 13px;display:'.$privateDropDown.'" 
		  onclick='.$postPublicArrayOnClick.'> Set private </a>';
	    	...
	    	$postArray['method'] = 'approvepost';
	    	$postArray['val'] = 1;
	    	$postArray['html'] = '<span class="badge badge-success">Visible</span>';
			$postApproveArrayOnClick = json_encode($postArray,JSON_UNESCAPED_UNICODE);
	        $postApproveArrayOnClick = "'adminUpdateData(".$postApproveArrayOnClick.")'"; 
	               	
	   		$approveDropdown.= '<a class="dropdown-item" data-post-dropdown-approve="'.$id.'" href="javascript:;" style="font-size: 13px;display:'.$approvedpost.'" 
		  onclick='.$postApproveArrayOnClick.'> Approve post</a>';        	
	    	      
	    	$postArray['method'] = 'approvepost';
	    	$postArray['val'] = 0;
	    	$postArray['html'] = '<span class="badge badge-warning">Pending Review</span>';
			$postApproveArrayOnClick = json_encode($postArray,JSON_UNESCAPED_UNICODE);
	        $postApproveArrayOnClick = "'adminUpdateData(".$postApproveArrayOnClick.")'"; 
	               	
	   		$approveDropdown.= '<a class="dropdown-item" data-post-dropdown-pending="'.$id.'" href="javascript:;" style="font-size: 13px;display:'.$pendingpost.'" 
		  onclick='.$postApproveArrayOnClick.'> Change to Pending</a>';        	
	        ...
	        if($value['visible'] == 2){
		        ...
		        $postApproveArrayOnClick = "'adminUpdateData(".$postApproveArrayOnClick.")'"; 
	       		...
	       		$approveDropdown.= '<a class="dropdown-item" data-post-dropdown-approve="'.$id.'" href="javascript:;" style="font-size: 13px;" 
			  onclick='.$postApproveArrayOnClick.'> Approve post </a>
			  <div class="dropdown-divider"></div>';
	        ...
	        $postArray['action'] = 'deletepost';
	        $deletepost = json_encode($postArray,JSON_UNESCAPED_UNICODE);
	        $deletepostOnClick = "'adminDeleteData(".$deletepost.")'";	


	        //DONT SHOW PRIVATE/PUBLIC IF TEXT
	        if($value['post_type'] == 'text'){
	        	$postPublicDropdown = '';
	        }
			...
			$dropdown = ' 		
			  <a class="dropdown-item" style="font-size: 13px" target="_blank" href="'.$sm['config']['site_url'].'@'.$value['uid'].'">Open live profile</a>
			  <a class="dropdown-item" style="font-size: 13px" href="'.$sm['config']['site_url'].'index.php?page=admin&p=user&id='.$value['uid'].'" target="_blank">Edit User</a>		  
			  <div class="dropdown-divider"></div>
			  '.$postPublicDropdown.'
			  '.$approveDropdown.'
			...
			$userTd = '
		        <td style="min-width:180px;cursor:pointer;" onclick="searchPostById('.$userId.')">
		            <div class="media align-items-center">
		                <div class="avatar avatar-sm mr-3" style="width:34px;height:34px;border-radius:50%">
		                    <img data-post-profile-photo="'.$userId.'" src="'.profilePhoto($userId).'" class="avatar-img rounded" style="border-radius:50%!important">
		                </div>
		                <div class="media-body">
		                    <strong class="js-lists-values-employee-name">'.$uName.'</strong><span class="badge badge-warning"></span><br>
	        ...
	        $data[$i]='
	            <tr class="data-search-verifications" data-post-i="'.$i.'" data-post-id="'.$id.'" data-post-type="'.$postType.'" data-post-src="'.$value['post_src'].'">
	                <td>
	                    <div class="custom-control custom-checkbox">
	                        <input type="checkbox" onclick="checkData(this,'.$checkData.')" class="custom-control-input" data-check-search="'.$checkData.'" id="checkcall_'.$id.'">
	                        <label class="custom-control-label" style="cursor: pointer;" for="checkcall_'.$id.'">
	                ...
	                <td style="min-width:110px">
	                	<small class="badge badge-'.$posttypeLabel.'">'.$postType.'</small>
	                </td> 
	                <td style="min-width:280px">
	                	<small>'.$value['post_meta'].'</small>
	                </td>                                
	                <td style="min-width:115px">
	                	<small class="text-muted">'.time_elapsed_string($timeago).'</small>
	                </td>  
	                <td style="min-width:85px">
	                	<small class="badge badge-'.$publicLabel.'" data-post-public="'.$id.'">
	                		'.$postPublic.'
	                	</small>
	                </td>
	                <td style="min-width:105px">
	                	<small class="text-muted" id="approvepost'.$id.'">'.$approved.'</small>
	                </td>
	                '.$userTd.'                                 
	                <td>
	                    <div class="dropdown ml-auto" data-table-dropdown>
	...
	case 'updatepost':
		$id = secureEncode($_POST['postId']);
		$type = secureEncode($_POST['postType']);	
		$method = secureEncode($_POST['method']);
		$updateData = '';
		if($method == 'postSetPrivate'){
			$updateData = 'post_premium = "Yes"';
		}
		if($method == 'postSetPublic'){
			$updateData = 'post_premium = "No"';
		}
		if($method == 'approvepost'){
			$val = secureEncode($_POST['val']);
			$updateData = 'visible = '.$val;
		}
		$mysqli->query('UPDATE feed SET '.$updateData.' WHERE id = "'.$id.'"');
	break;	

	case 'deletepost':
		$id = secureEncode($_POST['postId']);
		$mysqli->query("DELETE FROM feed where id = '".$id."'");
		$mysqli->query("DELETE FROM feed_comments where fid = '".$id."'");
		$mysqli->query("DELETE FROM feed_likes where fid = '".$id."'");
		$mysqli->query("DELETE FROM feed_reports where fid = '".$id."'");
		$mysqli->query("DELETE FROM users_feed_favs where fid = '".$id."'");
		$mysqli->query("DELETE FROM users_feed_purchases where fid = '".$id."'");
	break;
	...
	case 'search_date':
    	...
    	$table = 'dating';	
	    $order = 'id DESC';        	   
		$search = getArray($table,$filter,$order,'');
	    $i=0;
	    if(isset($search)){
	      foreach ($search as $value) { 
			...
			$arr['searchUserId'] = $value['uid'];
			$arr['searchUserName'] = getData('users','name','WHERE id = '.$value['uid']);
			$arr['searchGender'] = getData('users','gender','WHERE id = '.$value['uid']);
			

	      	$checkData = $value['id'];
	      	$id = $value['id'];
	      	$userId = $value['uid'];
	      	$time = $value['dating_time'];
	        ...
	        $deleteOnClick = "'deleteDateRequest(".$id.")'";	
			...
			$userTd = '
		        <td style="max-width:180px;cursor:pointer;">
		            <div class="media align-items-center">
		                <div class="avatar avatar-sm mr-3" style="width:34px;height:34px;border-radius:50%">
		                    <img src="'.profilePhoto($userId).'" class="avatar-img rounded" style="border-radius:50%!important">
		                </div>
		                <div class="media-body">
		                    <strong class="js-lists-values-employee-name">'.$uName.'</strong><span class="badge badge-warning"></span><br>
		                    <span class="text-muted js-lists-values-employee-title">ID: '.$userId.'</span>
	        ...
	        $data[$i]='
	            <tr class="data-search-verifications" data-date-request="'.$id.'" data-date-i="'.$i.'">
	                <td style="min-width:220px">
	                	<p>'.$value['dating_description'].'</p>
	                </td>  
	                <td style="min-width:80px;">
	                	<small>'.$value['dating_place'].'</small>
	                </td> 	
	                <td style="min-width:110px;">
	                	<small>'.str_replace('2023-','',$value['dating_when']).'</small>
	                </td>   	                                                              
	                <td style="min-width:80px;">
	                	<small>'.$value['dating_budget'].'</small>
	                </td>
	...
	case 'search_reel':
	    ...
	    $searchInput = secureEncode($_POST['search']);
	    $status = secureEncode($_POST['status']);
	    $visible = secureEncode($_POST['visible']);
	    ...
	    if(isset($search)){
	      foreach ($search as $value) { 
			...
			$play_video = "'".$value['reel_src']."'";
	      	$uName = getData('users','name','where id ='.$userId);
	      	$uploadedPhotoOnClick = "'".$value['reel_src']."',".$userId.",'".$uName."','".$uEmail."','".$uPhoto."'";
        	...
        	<td onclick="playVideo('.$play_video.')" style="position:relative;cursor:pointer;max-width:70px">
            	<video class="avatar avatar-img rounded avatar-lg mr-3 media-loading"
            	onclick="playVideo('.$play_video.')" style="width:70px;height:90px;cursor:pointer;border:0px solid #fff">
            		<source src="'.$value['reel_src'].'"/>
            	</video>
            	<div style="position:absolute;bottom:10px;left:15px;z-index:9">
            		<i class="material-icons" style="color:#fff;font-size:24px;cursor:pointer" onclick="playVideo('.$play_video.')">play_arrow</i>
            	</div>
	        ...
	        $reelArray = array();
	        $reelArray['reelId'] = $value['id'];
	        $reelArray['reelUid'] = $value['uid'];
	        $reelArray['reelSrc'] = $value['reel_src'];
	        $reelArray['reelMeta'] = $value['reel_meta'];
	        $reelArray['reelPrice'] = $value['reel_price'];

        	$checkReelPrice = $value['reel_price'];
        	if($checkReelPrice > 0){
        		$price = $checkReelPrice.'';
        	} else {
        		$price = 'FREE';   		
        	}

        	// Moderation status
        	$reelVisible = isset($value['visible']) ? $value['visible'] : 1;
        	$statusBadge = '';
        	$approveDropdown = '';
        	$pendingDropdown = '';
        	
        	if($reelVisible == 1){
        		$statusBadge = '<span class="badge badge-success" id="reelStatus'.$id.'" style="background-color: #28a745 !important; border: none !important; border-width: 0 !important; border-style: none !important; border-color: transparent !important; outline: none !important; box-shadow: none !important; color: #fff !important;">Approved</span>';
        		$pendingDropdown = 'block';
        		$approveDropdown = 'none';
        	} else if($reelVisible == 2){
        		$statusBadge = '<span class="badge badge-danger" id="reelStatus'.$id.'" style="background-color: #dc3545 !important; border: none !important; border-width: 0 !important; border-style: none !important; border-color: transparent !important; outline: none !important; box-shadow: none !important; color: #fff !important;">Rejected</span>';
        		$pendingDropdown = 'block';
        		$approveDropdown = 'none';
        	} else {
        		$statusBadge = '<span class="badge badge-warning" id="reelStatus'.$id.'" style="background-color: #ffc107 !important; border: none !important; border-width: 0 !important; border-style: none !important; border-color: transparent !important; outline: none !important; box-shadow: none !important; color: #000 !important;">Pending Review</span>';
        		$pendingDropdown = 'none';
        		$approveDropdown = 'block';
        	}

        	// Approve/Pending actions
        	$reelApproveArray = array();
        	$reelApproveArray['action'] = 'updateReel';
        	$reelApproveArray['method'] = 'approveReel';
        	$reelApproveArray['reelId'] = $id;
        	$reelApproveArray['val'] = 1;
        	$reelApproveArray['html'] = '<span class="badge badge-success" style="background-color: #28a745 !important; border: none !important; border-width: 0 !important; border-style: none !important; border-color: transparent !important; outline: none !important; box-shadow: none !important; color: #fff !important;">Approved</span>';
        	$reelApproveArrayOnClick = json_encode($reelApproveArray,JSON_UNESCAPED_UNICODE);
        	$reelApproveArrayOnClick = "'adminUpdateData(".$reelApproveArrayOnClick.")'";

        	$reelPendingArray = array();
        	$reelPendingArray['action'] = 'updateReel';
        	$reelPendingArray['method'] = 'approveReel';
        	$reelPendingArray['reelId'] = $id;
        	$reelPendingArray['val'] = 0;
        	$reelPendingArray['html'] = '<span class="badge badge-warning" style="background-color: #ffc107 !important; border: none !important; border-width: 0 !important; border-style: none !important; border-color: transparent !important; outline: none !important; box-shadow: none !important; color: #000 !important;">Pending Review</span>';
        	$reelPendingArrayOnClick = json_encode($reelPendingArray,JSON_UNESCAPED_UNICODE);
        	$reelPendingArrayOnClick = "'adminUpdateData(".$reelPendingArrayOnClick.")'";
        	
        	$reelArray['action'] = 'updateMedia';
        	$reelArray['method'] = 'changeReelPrice';
			$reelArrayOnClick = json_encode($reelArray,JSON_UNESCAPED_UNICODE);
	        $reelArrayOnClick = "'adminUpdateData(".$reelArrayOnClick.")'";


       		$reelPriceDropdown = '
       		<div class="dropdown-item" data-media-dropdown-price-reel="'.$id.'"
       		 style="font-size: 13px;">Reel price</div>
       			<input class="form-control" type="number" id="reelPriceInput'.$id.'" 
       			style="width:80%!important;margin-left:10%" onkeyup='.$reelArrayOnClick.' value="'.$checkReelPrice.'">

		  	<div class="dropdown-divider" data-media-dropdown-price-reel="'.$id.'"></div>';             	
	              

	        $reelArray['action'] = 'deletereel';
	        $deletereel = json_encode($reelArray,JSON_UNESCAPED_UNICODE);
	        $deletereelOnClick = "'adminDeleteData(".$deletereel.")'";	
			...
			$dropdown = ' 		
			  ...
			  <a class="dropdown-item" style="font-size: 13px" href="'.$sm['config']['site_url'].'index.php?page=admin&p=user&id='.$value['uid'].'" target="_blank">Edit User</a>		  	
			  <div class="dropdown-divider"></div>
			  <a class="dropdown-item" data-reel-dropdown-approve="'.$id.'" href="javascript:;" style="font-size: 13px;display:'.$approveDropdown.';color: #09c66e" 
			  onclick='.$reelApproveArrayOnClick.'> Approve reel</a>
			  <a class="dropdown-item" data-reel-dropdown-pending="'.$id.'" href="javascript:;" style="font-size: 13px;display:'.$pendingDropdown.';color: #ff9800" 
			  onclick='.$reelPendingArrayOnClick.'> Set to Pending</a>
			  <div class="dropdown-divider"></div>	  
			  '.$reelPriceDropdown.'
			  <a class="dropdown-item" href="#" style="font-size: 13px;color: #b50000" 
			  onclick='.$deletereelOnClick.'>
			  Delete reel</a>'; 

			$userTd = '
		        <td style="min-width:180px;cursor:pointer;" onclick="searchReelById('.$userId.')">
		            <div class="media align-items-center">
		                <div class="avatar avatar-sm mr-3" style="width:34px;height:34px;border-radius:50%">
		                    <img data-reel-profile-photo="'.$userId.'" src="'.profilePhoto($userId).'" class="avatar-img rounded" style="border-radius:50%!important">
		                </div>
		                <div class="media-body">
		                    <strong class="js-lists-values-employee-name">'.$uName.'</strong><span class="badge badge-warning"></span><br>
		                    <span class="text-muted js-lists-values-employee-title">ID: '.$userId.'</span>
			...
			// Rekognition data column
			$rekognitionData = isset($value['rekognition']) ? $value['rekognition'] : '';
			$rekognitionDisplay = '<small class="text-muted">No data</small>';
			
			if(!empty($rekognitionData)){
				// Try to decode JSON
				$rekognitionJson = json_decode($rekognitionData, true);
				if(json_last_error() === JSON_ERROR_NONE && is_array($rekognitionJson)){
					// Format JSON data nicely
					$rekognitionDisplay = '<div style="font-size:11px;line-height:1.4;">';
					if(isset($rekognitionJson['ModerationLabels'])){
						foreach($rekognitionJson['ModerationLabels'] as $label){
							if(isset($label['Name']) && isset($label['Confidence'])){
								$confidence = round($label['Confidence'], 1);
								$color = $confidence >= 80 ? 'danger' : ($confidence >= 60 ? 'warning' : 'success');
								$rekognitionDisplay .= '<span class="badge badge-'.$color.'" style="margin:2px;font-size:10px;">'.$label['Name'].': '.$confidence.'%</span><br>';
							}
						}
					} else {
						// If not in expected format, show formatted JSON
						$rekognitionDisplay .= '<pre style="font-size:10px;max-height:100px;overflow:hidden;margin:0;padding:5px;background:#f5f5f5;border-radius:3px;">'.htmlspecialchars(json_encode($rekognitionJson, JSON_PRETTY_PRINT)).'</pre>';
					}
					$rekognitionDisplay .= '</div>';
				} else {
					// If not JSON, display as text
					$rekognitionDisplay = '<small style="font-size:11px;word-break:break-word;">'.htmlspecialchars(substr($rekognitionData, 0, 200)).(strlen($rekognitionData) > 200 ? '...' : '').'</small>';
				}
			}

			$rekognitionTd = '
		        <td style="min-width:250px;max-width:250px;padding:8px;">
		            '.$rekognitionDisplay.'
		        </td> 
			';
	        ...
	        $data[$i]='
	            <tr class="data-search-verifications" data-reel-i="'.$i.'" data-reel-id="'.$id.'" data-reel-src="'.$value['reel_src'].'">
	                '.$searchreel.'
	                <td style="min-width:180px">
                       <textarea style="width:100%;font-size:13px;line-height: 18px;height:60px;background: none;color:#939fad;border:none;outline: none;" id="editReelTextarea'.$value['id'].'" onchange="editReelAdmin('.$id.',this.value,'.$value['reel_price'].')">'.$value['reel_meta'].'</textarea>                
	                </td>  
	                <td style="min-width:65px;text-align:center">
	                	<small id="reelPrice'.$id.'">'.$price.'</small>
	                </td>
	                <td style="min-width:100px;text-align:center">
	                	'.$statusBadge.'
	                </td> 	
	                <td style="min-width:180px;text-align:center">
	                	<small>'.$value['viewed'].' / '.$likes.' / '.$value['purchased'].'</small>
	                </td>  	                                                              
	                <td style="min-width:115px">
	                	<small class="text-muted">'.time_elapsed_string($timeago).'</small>
	                </td>  
    ...
    // Flag to show rekognition column only for story media type
    $showRekognition = ($mediatype == 'story');
    ...
    if(isset($search)){
      foreach ($search as $value) { 
        ...
        // For stories, get the actual story type (image or video) from users_story table
        if($value['story'] > 0){
        	$storyType = getData('users_story','storyType','WHERE id = '.$value['story']);
        	if($storyType != 'noData' && !empty($storyType)){
        		if(strtolower($storyType) == 'video'){
        			$mediaType = 'Video';
        			$mediatypeLabel = 'dark';
        		} else {
        			$mediaType = 'Image';
        		...
        		// Fallback to video column if story type not found
        		if($value['video'] == 1){
        			$mediaType = 'Video';
        			$mediatypeLabel = 'dark';
        		} else {
        ...
        // For stories, use visible column from users_story table
        // For other media, use approved column from users_photos table
        if($value['story'] > 0){
        	// Get visible status from users_story table
        	$storyVisible = getData('users_story','visible','WHERE id = '.$value['story']);
        	if($storyVisible == '' || $storyVisible == 'noData'){
        		$storyVisible = 0; // Default to pending if not set
        	}
        	
        	$statusBadge = '';
        	$approveDropdownStory = '';
        	$rejectDropdownStory = '';
        	
        	if($storyVisible == 1){
        		$statusBadge = '<span class="badge badge-success" id="storyStatus'.$id.'" style="background-color: #28a745 !important; border: none !important; border-width: 0 !important; border-style: none !important; border-color: transparent !important; outline: none !important; box-shadow: none !important; color: #fff !important;">Approved</span>';
        		$approveDropdownStory = 'none';
        		$rejectDropdownStory = 'block';
        	} else if($storyVisible == 2){
        		$statusBadge = '<span class="badge badge-danger" id="storyStatus'.$id.'" style="background-color: #dc3545 !important; border: none !important; border-width: 0 !important; border-style: none !important; border-color: transparent !important; outline: none !important; box-shadow: none !important; color: #fff !important;">Rejected</span>';
        		$approveDropdownStory = 'block';
        		$rejectDropdownStory = 'none';
        	} else {
        		$statusBadge = '<span class="badge badge-warning" id="storyStatus'.$id.'" style="background-color: #ffc107 !important; border: none !important; border-width: 0 !important; border-style: none !important; border-color: transparent !important; outline: none !important; box-shadow: none !important; color: #000 !important;">Pending Review</span>';
        		$approveDropdownStory = 'block';
        		$rejectDropdownStory = 'block';
        	}
        	...
        	// Approve action
        	$storyApproveArray = array();
        	$storyApproveArray['action'] = 'updateMedia';
        	$storyApproveArray['method'] = 'approveStory';
        	$storyApproveArray['mediaId'] = $id;
        	$storyApproveArray['mediaIdStory'] = $value['story'];
        	$storyApproveArray['val'] = 1;
        	$storyApproveArray['html'] = '<span class="badge badge-success" style="background-color: #28a745 !important; border: none !important; border-width: 0 !important; border-style: none !important; border-color: transparent !important; outline: none !important; box-shadow: none !important; color: #fff !important;">Approved</span>';
        	$storyApproveArrayOnClick = json_encode($storyApproveArray,JSON_UNESCAPED_UNICODE);
        	$storyApproveArrayOnClick = "'adminUpdateData(".$storyApproveArrayOnClick.")'";
        	
        	// Reject action
        	$storyRejectArray = array();
        	$storyRejectArray['action'] = 'updateMedia';
        	$storyRejectArray['method'] = 'approveStory';
        	$storyRejectArray['mediaId'] = $id;
        	$storyRejectArray['mediaIdStory'] = $value['story'];
        	$storyRejectArray['val'] = 2;
        	$storyRejectArray['html'] = '<span class="badge badge-danger" style="background-color: #dc3545 !important; border: none !important; border-width: 0 !important; border-style: none !important; border-color: transparent !important; outline: none !important; box-shadow: none !important; color: #fff !important;">Rejected</span>';
        	$storyRejectArrayOnClick = json_encode($storyRejectArray,JSON_UNESCAPED_UNICODE);
        	$storyRejectArrayOnClick = "'adminUpdateData(".$storyRejectArrayOnClick.")'";
        	
        	// Pending action
        	$storyPendingArray = array();
        	$storyPendingArray['action'] = 'updateMedia';
        	$storyPendingArray['method'] = 'approveStory';
        	$storyPendingArray['mediaId'] = $id;
        	$storyPendingArray['mediaIdStory'] = $value['story'];
        	$storyPendingArray['val'] = 0;
        	$storyPendingArray['html'] = '<span class="badge badge-warning" style="background-color: #ffc107 !important; border: none !important; border-width: 0 !important; border-style: none !important; border-color: transparent !important; outline: none !important; box-shadow: none !important; color: #000 !important;">Pending Review</span>';
        	$storyPendingArrayOnClick = json_encode($storyPendingArray,JSON_UNESCAPED_UNICODE);
        	$storyPendingArrayOnClick = "'adminUpdateData(".$storyPendingArrayOnClick.")'";
        	
        	$approveDropdown.= '<a class="dropdown-item" data-story-dropdown-approve="'.$id.'" href="javascript:;" style="font-size: 13px;display:'.$approveDropdownStory.'" 
		  onclick='.$storyApproveArrayOnClick.'> Approve story</a>';
        	$approveDropdown.= '<a class="dropdown-item" data-story-dropdown-reject="'.$id.'" href="javascript:;" style="font-size: 13px;display:'.$rejectDropdownStory.'" 
		  onclick='.$storyRejectArrayOnClick.'> Reject story</a>';
        	$approveDropdown.= '<a class="dropdown-item" data-story-dropdown-pending="'.$id.'" href="javascript:;" style="font-size: 13px;display:'.($storyVisible == 1 || $storyVisible == 2 ? 'block' : 'none').'" 
		  onclick='.$storyPendingArrayOnClick.'> Change to Pending</a>';
        } else {
        	// For non-story media, use the existing approved column logic
        	$approved = '<span class="badge badge-success">Visible</span>';
	  ...
	  onclick='.$mediaApproveArrayOnClick.'> Change to Pending</a>';        	
		...
		// Check if story is still visible (within time limit and approved)
		if($value['story'] > 0){
			$storiesFilter = 'where id = '.$value['story'];
			$checkStoryTime = getData('users_story','storyTime',$storiesFilter);
			$checkStoryVisible = getData('users_story','visible',$storiesFilter);
			
			// Only show re-upload option if story is approved (visible = 1) but expired
			if($checkStoryVisible == 1 && $checkStoryTime <= $storyFrom){
				$mediaArray['action'] = 'updateMedia';
		...
		// Rekognition data column - only show when mediatype is 'story'
		$rekognitionTd = '';
		if(isset($showRekognition) && $showRekognition){
			$displayStyle = 'style="min-width:250px;max-width:250px;padding:8px;"';
			if($value['story'] > 0){
				$rekognitionData = getData('users_story','rekognition','WHERE id = '.$value['story']);
				$rekognitionDisplay = '<small class="text-muted">No data</small>';
				
				if(!empty($rekognitionData) && $rekognitionData != 'noData' && $rekognitionData != 'NULL' && strtolower(trim($rekognitionData)) != 'null'){
					// Try to decode JSON
					$rekognitionJson = json_decode($rekognitionData, true);
					if(json_last_error() === JSON_ERROR_NONE && is_array($rekognitionJson)){
						// Show formatted JSON in pre tag like reels page
						$rekognitionDisplay = '<pre style="font-size:10px;max-height:150px;overflow:hidden;margin:0;padding:5px;background:#f5f5f5;border-radius:3px;">'.htmlspecialchars(json_encode($rekognitionJson, JSON_PRETTY_PRINT)).'</pre>';
					} else {
						// If not JSON, display as text
						$rekognitionDisplay = '<small style="font-size:11px;word-break:break-word;">'.htmlspecialchars(substr($rekognitionData, 0, 200)).(strlen($rekognitionData) > 200 ? '...' : '').'</small>';
					}
				...
				$rekognitionTd = '
			        <td data-rekognition-cell '.$displayStyle.'>
			            '.$rekognitionDisplay.'
			        </td> 
				';
			} else {
				// For non-story media when viewing story filter, show empty cell
				$rekognitionTd = '
			        <td data-rekognition-cell '.$displayStyle.'>
			...
			// Hide rekognition cell when not viewing stories
			$rekognitionTd = '
		        <td data-rekognition-cell style="display:none;min-width:250px;max-width:250px;padding:8px;">
		        </td> 
			';
		}
	...
	case 'updateMedia':
		...
		$method = secureEncode($_POST['method']);
		if($method == 'changeReelPrice'){
			$val = secureEncode($_POST['val']);
			$reelId = secureEncode($_POST['reelId']);
			$mysqli->query('UPDATE reels SET reel_price = '.$val.' WHERE id = "'.$reelId.'"');
			exit;
		}	
		...
		if($method == 'uploadToStory'){
			...
			$fake = getData('users','fake','WHERE id ='.$uid);
      		$query = "INSERT INTO users_story (uid,storyTime,story,storyType,lat,lng,visible)
      			 VALUES ('".$uid."','".$time."','".$p."','".$video."','".$lat."','".$lng."',0)";
      		if ($mysqli->query($query) === TRUE) {
      			$last_id = $mysqli->insert_id;
				if($type == 'Video'){
					$video = 1;
		...
		if($method == 'approveMedia'){
			...
			// Note: Story approval is now handled by approveStory method
		}
		
		if($method == 'approveStory'){
			$val = secureEncode($_POST['val']); // 0 = pending, 1 = approve, 2 = rejected
			$storyId = secureEncode($_POST['mediaIdStory']);
			$mysqli->query('UPDATE users_story SET visible = '.$val.' WHERE id = "'.$storyId.'"');
			$arr = array();
			$arr['OK'] = 'OK';
			$arr['storyId'] = $storyId;
			$arr['val'] = $val;
    ...
    $order = 'order_date';
    $filter = 'where order_status = "pending" and order_custom = 1';
    if(!empty($_POST['search'])){
    	$s = secureEncode($_POST['search']);
    	$filter = 'WHERE order_id = "'.$s.'"';
    }
    $search = getArray('orders',$filter,$order.' desc','');
    $i=0;
    if(isset($search)){
      foreach ($search as $value) { 
		...
		$proof_image = '<a href="'.$value['order_proof_img'].'" target="_blank"><img style="max-width:120px;height:auto;" src="'.$value['order_proof_img'].'" class="avatar-img square"></a>';
        ...
        $onclickApprove = "adminPendingOrder(1,'".$order_id."')";
        $onclickDelete = "adminPendingOrder(2,'".$order_id."')";
		$dropdown= '			  
			<a class="dropdown-item" href="javascript:;" 
            ...
            <tr>
                <td>
                    <div class="media align-items-center">
                        ...
                        <div class="media-body">
                        	<a href="'.$sm['config']['site_url'].'index.php?page=admin&p=user&id='.$value['user_id'].'" target="_blank" style="text-decoration:none;color:auto">
                            <strong>'.getData('users','name','where id ='.$value['user_id']).'</strong> 
                            </a><br>
							<span><small>'.$value['order_id'].'</small></span>                            
                        </div>
                    </div>
                </td>            
            	<td><small class="text-muted">'.$price.'<br>'.$actionText.'</small></td>
                <td><small>'.ucfirst($value['order_gateway']).'</small></td>
                <td><small class="text-muted">'.$value['order_proof'].'</small></td>
				<td>'.$proof_image.'</td>
				<td><small class="text-muted">'.date('d-m h:m',$value['order_date']).'</small></td>
                <td>
                    <div class="dropdown ml-auto">
                        <a href="#" data-toggle="dropdown" data-caret="false" class="btn btn-light text-muted"><i class="material-icons">more_vert</i></a>
                        <div class="dropdown-menu dropdown-menu-right">                          
	...
	case 'pending_order':
		$order_id = secureEncode($_POST['order_id']);
		$a = secureEncode($_POST['a']);
		...
		if($a == 1){
			...
			if (!empty($sm['plugins']['email']['host'])) { 			
				paymentApprovedMailNotification($order_id,$actionText);
			}            
			...
			if($sm['plugins']['logActivity']['enabled'] == 'Yes'){ 
				$activity = 'Order '.$order_id.' has been deleted by '.$sm['user']['name'];
				activity('system',$activity,'Pending order deleted '.$order_id);	
			}
		}	
		$arr = array();
		$arr['OK'] = 'OK';		
    ...
    case 'deletereel':
        $id = secureEncode($_POST['reelId']);
        deleteData('reels','WHERE id = '.$id);
	...
	case 'updateReel':
		$method = secureEncode($_POST['method']);
		if($method == 'approveReel'){
			$val = secureEncode($_POST['val']);
			$reelId = secureEncode($_POST['reelId']);
			$mysqli->query('UPDATE reels SET visible = '.$val.' WHERE id = "'.$reelId.'"');
			$arr = array();
			$arr['OK'] = 'OK';
			$arr['reelId'] = $reelId;
			$arr['val'] = $val;
