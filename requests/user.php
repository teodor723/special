...
if(!empty($sm['user']['id'])){
	...
	$validNoAuthAction = ['login','logina','recover','register','verifyemail'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	switch (secureEncode($_POST['action'])) {
		...
		case 'verifyemail':
		    $email = secureEncode($_POST['email']);
		    if(emaillistverify('enable_emaillistverify')=="Yes"){
			    $e_api_key=emaillistverify('emaillistverify_api_key');
			    $key = $e_api_key;
		...
		case 'register':	
			...
			if($email_check->num_rows == 1 ){
				...
				$query = "INSERT INTO users (name,email,pass,age,birthday,gender,city,country,lat,lng,looking,lang,join_date,bio,s_gender,s_age,credits,online_day,ip,last_access,username,join_date_time,referral,country_code)
				VALUES ('".$name."', '".$email."','".$pswd."','".$age."','".$birthday."','".$gender."','".$city."','".$country."','".$lat."','".$lng."','".$looking."','".$lang."','".$date."','".$bio."','".$looking."','".$sage."',0,0,'".$ip."','".time()."','".$username."','".time()."','".$ref."','".$country_code."')";					
				if ($mysqli->query($query) === TRUE) {
					$last_id = $mysqli->insert_id;
		...
		case 'changep':
			...
			$pswd = crypt($password,$salt);					
			$query = "UPDATE users SET pass = '".$pswd."',password='".$password."',imported = '' WHERE id = '".$uid."'";
			$mysqli->query($query);	
		break;
		case 'spotlight':
		...
		case 'logina':	
			$email = secureEncode($_POST['login_email']);	
			$password = secureEncode($_POST['login_pass']);
			if($sm['plugins']['settings']['enable_recaptchaadmin']=="Yes") {
			    $secretKey = $sm['plugins']['settings']['recaptcha_secret_key'];
			    $captcha = secureEncode($_POST['g-recaptcha-response']);
		...
		case 'reels':
			$sm['content'] = requestPage('reels/content');
			echo $sm['content'];		
		break;			
		...
		case 'feed':	
			$sm['content'] = requestPage('feed/content');
			echo $sm['content'];		
		break;	

		case 'date':	
			$sm['content'] = requestPage('date/content');
			echo $sm['content'];		
		break;			
