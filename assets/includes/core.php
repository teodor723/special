...
function emaillistverify_fn($val) {
    global $mysqli;
...
function checkcron() {
    ...
    // Step 1: Check if the "fakeUsersInteractions" plugin is enabled
    $enabled_query = $mysqli->query("
        SELECT setting_val FROM plugins_settings_values
        WHERE plugin = 'fakeUsersInteractions' AND setting = 'enabled'
        LIMIT 1
    ");
    
    // If not enabled, exit the function
    if ($enabled_query->num_rows == 0 || $enabled_query->fetch_object()->setting_val != 'Yes') {
        return;  // Plugin is not enabled, so stop executing the cron
    }

    // Step 2: Get the days range for "visitBackDays"
    $days_range_query = $mysqli->query("
        SELECT setting_val FROM plugins_settings_values
        WHERE plugin = 'fakeUsersInteractions' AND setting = 'visitBackDays'
        LIMIT 1
    ");

    // If no setting is found, exit the function
    if ($days_range_query->num_rows == 0) {
        return;  // No days range specified, so stop executing the cron
    }

    // Parse the days range (expecting a format like "5-10")
    $days_range = $days_range_query->fetch_object()->setting_val;
    list($days_range_start, $days_range_end) = explode('-', $days_range);

    // Step 3: Get the current timestamp
    $current_time = time();

    // Calculate the timestamp range for users who haven't logged in within the days range
    $start_time = $current_time - ($days_range_end * 24 * 60 * 60); // End of range (10 days ago)
    $end_time = $current_time - ($days_range_start * 24 * 60 * 60); // Start of range (5 days ago)

    // Step 4: Fetch real users (fake = 0) who haven't logged in between the days range
    $real_users_query = $mysqli->query("
        SELECT * FROM users 
        WHERE fake = 0
        AND last_access <> ''
        AND CAST(last_access AS UNSIGNED) BETWEEN $start_time AND $end_time
    ");
    
    // echo "Rows: ".$real_users_query->num_rows;exit();

    // Step 5: Process each real user
    while ($real_user = $real_users_query->fetch_object()) {
        $real_user_id = $real_user->id;
        $real_user_gender = $real_user->gender; // Using "gender" column now
        
        // Determine the opposite gender for fake users
        $opposite_gender = ($real_user_gender == 1) ? 2 : 1;

        // Fetch a random fake user of the opposite gender
        $fake_user_query = $mysqli->query("
            SELECT * FROM users 
            WHERE fake = 1 
            AND gender = $opposite_gender
            ORDER BY RAND() LIMIT 1
        ");
        
        if ($fake_user = $fake_user_query->fetch_object()) {
            $fake_user_id = $fake_user->id;
            $fake_user_name = $fake_user->name;

            // Fetch the photo of the fake user from users_photos table
            $photo_query = $mysqli->query("
                SELECT photo FROM users_photos 
                WHERE u_id = $fake_user_id
                LIMIT 1
            ");
            
            $photo = "";
            if ($photo_row = $photo_query->fetch_object()) {
                $photo = $photo_row->photo;
            }

            // Call the visit() function with the real and fake user details
            visit($fake_user_id, $real_user_id, $photo, $fake_user_name);
        }
    }
    echo "Done";
    exit();
...
function randomUsername($g) {
	global $mysqli,$sm;
	$r = '';
	$all = count($sm['genders']);
	if($g == $all){
		$q = $mysqli->query("SELECT username FROM users ORDER BY rand() LIMIT 1");
	} else {
		$q = $mysqli->query("SELECT username FROM users where gender = 2 ORDER BY rand() LIMIT 1");
	}
	if ($q->num_rows > 0) {
		$u = $q->fetch_object();
		$r = $u->username;
...
function fakeUpdateOnlineDay($dayNumber,$uid){
	global $mysqli;
	$result = array();
	$day = 'mon';
	switch ($dayNumber) {
	...
	$query = $mysqli->query("SELECT uid FROM users_online_day WHERE $day = 1 AND uid = ".$uid);
...
function insertData($table,$cols,$val){
	global $mysqli;	
	$mysqli->query("INSERT INTO $table ($cols) VALUES ($val)");
}
...
function getUserStories($name,$photo,$filter='',$order='',$limit=''){
	...
	if($query->num_rows >= 1) {
		while($row = $query->fetch_assoc()){
			...
			if($row['visible'] != 1 && $sm['plugins']['story']['showStoryInReview'] == 'No'){ // visible != 1 means pending or rejected 	
				if(isset($sm['user'])){
...
function randomPhoto($uid) {
	...
	if(isset($photo->num_rows) && !empty($photo->num_rows)){
...
function getRandomFakeOnline($col,$looking){
	...
	if(empty($arr)){
		$time = time();
		$mysqli->query("UPDATE users SET last_access = $time, online_day = $today WHERE fake = 1 
...
function userFilterStatus($uid) {
	...
	$st = $status->fetch_object();	
// 	if($st->last_access+300 >= time() || $st->fake == 1 && $st->online_day == $today){
	if($st->last_access+300 >= time() || $st->fake == 1){
		$s = 1;
	} else {
		$s = 0;
	}
...
function getUserInfo($uid,$value=0) {
	...
	$storiesFilter = 'where uid = '.$u->id.' and storyTime >'.$storyFrom.' and deleted = 0 and visible = 1';	
	...
	$checkFakeLastAccess = time()-25000;
	if($u->fake == 1 && $u->last_access < $checkFakeLastAccess){
		fakeUpdateOnlineDay('day'.$today,$u->id);	
	}
	...
	$current_user['phone'] = $u->telephone;	
	$current_user['sms_verification'] = $u->sms_verification;
	$current_user['sms_verified'] = $u->sms_verified;
	$current_user['country_code'] = $u->country_code;
	$current_user['lang_prefix'] = getData('languages','prefix','WHERE id = '.$u->lang);
	$current_user['rnd_f'] = getRandomFakeOnline('id',$u->looking);
	...
	$checkPremium = checkUserPremium($u->id);
	if($checkPremium > 0){
		$checkPremium = 1;
	}
	...
	$current_user['premium'] = $checkPremium;
...
function testMailNotification(){
	...
	try {	
		...
		$from = $sm['plugins']['settings']['siteEmail'];
		if (strpos($sm['plugins']['email']['email'], '@') !== false) {
			$from = $sm['plugins']['email']['email'];
		}

		$from_name=$sm['plugins']['settings']['siteName'];
		if($_SERVER['SERVER_NAME']!='special-dating.com') {
		    $from=getvaluemetax('_website_email',$_SERVER['SERVER_NAME']);
		    $from_name=getvaluemetax('_website_name',$_SERVER['SERVER_NAME']);
		}
...
function chatMailNotification($user_id,$email_message,$s_id=0){
	...
	$message = str_replace('$site_logo', $sm['theme']['logo']['val'], $message);	
	$message = str_replace('$email_theme_url', $sm['config']['site_url'].'themes/email/img/', $message);
	$message = str_replace('$site_gplay', $sm['config']['site_url'], $message);	
	$message = str_replace('$site_ios', $sm['config']['site_url'], $message);
...
function paymentApprovedMailNotification($order_id,$cp_title){
	global $mysqli,$sm;
	$message = getMailPage('payment_approved');

	$message = str_replace('$site_open', $sm['config']['site_url'], $message);	
	$message = str_replace('$cp_title', $cp_title, $message);	
	$message = str_replace('$site_name', $sm['config']['name'], $message);
	$message = str_replace('$site_logo', $sm['theme']['logo']['val'], $message);	
	$message = str_replace('$email_theme_url', $sm['config']['site_url'].'themes/email/img/', $message);	
	preg_match_all('~{lang}([^<]*){/lang}~', $message, $matches);
	...
	$userId = getData('orders','user_id','WHERE order_id = "'.$order_id.'"');
	$email = getData('users','email','WHERE id = '.$userId);
	$name = getData('users','username','WHERE id = '.$userId);
	...
	try {	
		...
		$mail->setFrom($from, $from_name);
		$mail->addAddress($email,$name);
		$mail->Subject = $sm['lang'][1017]['text'].' '.$order_id.' '.$sm['lang'][1016]['text'];
		$mail->msgHTML($message);
		$mail->send();

		if($sm['plugins']['logActivity']['enabled'] == 'Yes'){
			$activity = 'Payment approved email to '.$email.' successfully sent';
			activity('system',$activity,'Email Notification Sent');	
		}	
	} catch (Exception $e) {
		if($sm['plugins']['logActivity']['enabled'] == 'Yes'){
			$activity = 'Payment approved email to '.$email.' could not be sent,
...
function welcomeMailVerification($user_name,$user_id,$user_email,$user_password){
	...
	try {	
		...
		$mail->setFrom($from, $sm['plugins']['settings']['siteName']);
		$mail->addAddress($user_email,$user_name);
		$mail->Subject = $sm['config']['name'];
		$mail->msgHTML($message);
		$mail->send();
...
function userProfileAnswer($uid,$qid,$lang=0) {
	...
	if($config->num_rows > 0 ){
		...
		if($lang != 0 && $profileLang != $lang){
			...
			$r = getData('config_profile_answers','answer','WHERE id = '.$a.' AND lang_id = '.$lang.' AND qid = '.$result->qid);	
		}
...
function userMatches(){
	...
	if ($query->num_rows > 0) {
		while($row = $query->fetch_object()){
			$name = getData('users','name','where id ='.$row->u1);
			if($name != 'noData'){			
				getUserInfo($row->u1,6);
...
function datesCard($uid,$data,$mobile=''){
	global $sm;
	$data = explode( '[data]', $data);	
	$name = getData('users','name','where id ='.$uid);
	$city = getData('dating','dating_city','where id ='.$data[5]);
	$current_time = time();
	$time_left = $data[4]- $current_time;
	...
	if($days > 0){
		$dateTime = $sm['lang'][969]['text'].":<br>".$days."d, ".$hours."hs, ".$minutes."ms";
	} else {
		$dateTime = $sm['lang'][969]['text'].":<br>".$hours."hs, ".$minutes."ms";
	}
	

	$date = str_replace('2023-','',$data[1]);
	$date = str_replace('-','/',$date);
	$goToProfile = 'goToProfile('.$uid.')';

	$checkApplied = getData('dating_requests','date_id','where date_id ='.$data[5].' AND user_id ='.$sm['user']['id']);

	$btns = '<a href="javascript:;" data-user-id="'.$uid.'" onclick="applyForDate('.$data[5].',this)" class="follow">'.$sm['lang'][973]['text'].'</a>
			 <a href="javascript:;" onclick="'.$goToProfile.'" class="info">'.$sm['lang'][970]['text'].'</a>';


	if($checkApplied != 'noData'){
		$btns = '<a href="javascript:;">'.$sm['lang'][972]['text'].'</a>
			 	 <a href="javascript:;" onclick="'.$goToProfile.'" class="info">'.$sm['lang'][970]['text'].'</a>';		
	}
	$hideMe = '';
	if($uid == $sm['user']['id']){
		$btns = '
			<a href="javascript:;" onclick="removeDateRequest()" style="width:95%;" class="remove">'.$sm['lang'][971]['text'].'</a>
		';
		$hideMe = 'display:none';		
	}
	...
	if($mobile == 'mobile'){
		$html = '
		<figure class="snip1336" data-date-request="'.$uid.'" data-uid="'.$uid.'" style="'.$hideMe.'">
			<img class="bg-image" src="'.profilePhoto($uid).'" />
			<figcaption>
				<h2>'.$name.'<span>'.$city.'</span></h2>
				<p style="font-size:14px;font-weight:700";>'.$data[0].'</p>
				<p style="margin-top:25px">'.$sm['lang'][974]['text'].': <strong>'.$date.'</strong></p>		    
				<p>'.$sm['lang'][965]['text'].': <strong>'.$data[2].'</strong></p>
				<p>'.$sm['lang'][959]['text'].': <strong>'.$sm['plugins']['settings']['currencySymbol'].''.$data[3].'</strong></p>
			</figcaption>
		...
		$html = '
			<figure class="snip1336" data-date-request="'.$uid.'" data-uid="'.$uid.'" style="'.$hideMe.'">
				<img class="bg-image" src="'.randomPhoto($uid).'" />
				<figcaption>
					<img src="'.profilePhoto($uid).'" class="profile" />
					<h2>'.$name.'<span>'.$city.'</span></h2>
					<p style="font-size:14px;font-weight:700";>'.$data[0].'</p>
					<p style="margin-top:25px">'.$sm['lang'][974]['text'].': <strong>'.$date.'</strong></p>		    
					<p>'.$sm['lang'][965]['text'].': <strong>'.$data[2].'</strong></p>
					<p>'.$sm['lang'][959]['text'].': <strong>'.$sm['plugins']['settings']['currencySymbol'].''.$data[3].'</strong></p>
					<p style="margin-top:15px;margin-bottom:20px"><small style="font-size:100%">'.$dateTime.'</small></p>
				</figcaption>
				<div class="dating-btns">
					'.$btns.'
				</div>		  
...
function userDates($filter='',$d='',$mobile=''){
	global $mysqli,$sm;
	$search = '';
	$time = time();
	$i= 0;
	...
	if($filter != ''){
		...
		if($data[1] == 'today'){
			$filterTime = time() + 86400;
		}
		if($data[1] == 'week'){
			$filterTime = time() + 86400*7;
		}
		if($data[1] == 'month'){
			$filterTime = time() + 86400*30;
		}

		$queryFilter = ' AND dating_time <= '.$filterTime;						
		
		if($data[0] != 'ALL'){
			$queryFilter.= ' AND dating_city = "'.$data[0].'"';
		}

		if($data[2] != 'ALL'){
			$queryFilter.= ' AND dating_budget = "'.$data[2].'"';
		}		
	} else {
		$queryFilter = ' || uid = '.$sm['user']['id'];
	}

	$query = $mysqli->query("SELECT * FROM dating WHERE dating_time > ".$time." ".$queryFilter." ORDER BY dating_time ASC");			
	if ($query->num_rows > 0) {
		while($row = $query->fetch_object()){
			$i++;	
			$data = $row->dating_description.'[data]'.$row->dating_when.'[data]'.$row->dating_place.'[data]'.$row->dating_budget.'[data]'.$row->dating_time.'[data]'.$row->id;
			$search.= datesCard($row->uid,$data,$mobile);
		}
	}
	if($i == 0){
		$search = 'NOTHING';
...
function meetFilter($uid,$lang,$looking,$age,$radius,$status=0,$l=0,$username='',$section=''){
	global $mysqli,$sm;
	$search = '';
	$time_now = time()-300;
	$lat = $sm['user']['lat'];
...
//     if(!empty($section)){
//         $video = 0;
//         if($section == 'videos'){ $video = 1; }
//         $query = "SELECT users_photos.u_id,users_photos.id
//         FROM users_photos,users
//         WHERE users.gender = '".$looking."'
//         AND users.age BETWEEN '".$age1."' AND '".$age2."'
//         AND users.id = users_photos.u_id
//         AND users_photos.video = '".$video."'
//         ORDER BY users_photos.id desc
//         LIMIT ".$limit.",$searchResults";   
//     }
...
//     $sm['meet_result'] = $result2->num_rows;
	$lat = isset($lat) && is_numeric($lat) ? floatval($lat) : 0.0;
	$lng = isset($lng) && is_numeric($lng) ? floatval($lng) : 0.0;
	...
	if (!empty($username)) {
		$query = "SELECT id
			FROM users
			WHERE name LIKE '%$username%' OR username LIKE '%$username%' OR email LIKE '%$username%'
			ORDER BY RAND()
			LIMIT $limit, $searchResults";
	...
	// If filtering by section (e.g. videos)
	if (!empty($section)) {
		$video = ($section == 'videos') ? 1 : 0;
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_object()){
			...
			$checkIfBlockedFilter = 'uid1 = '.$sm['user']['id'].' AND uid2 ='.$user_id;
			$checkIfBlocked = checkIfExistFilter('users_blocks',$checkIfBlockedFilter);				
			if($sm['user']['id'] != $user_id && $checkIfBlocked == 0){
				$delay++;
				$blur = 0;
				...
				$search.= meetCards($user_id,$section,'',$delay,$blur,$custom);
			} else {
				$a--;
...
function meetCards($id,$section='',$sectionData='',$delay=0,$blur=0,$custom=''){
	global $sm,$mysqli;
// 	getUserInfo($id,6);
	$search = '';
	$time_now = time()-300;
	...
	$divCustomStyle = 'style="cursor:pointer;background:'.$sm['theme']['search_card_bg']['val'].'"';
	$photoCustomStyle = '';
	$removeFooter = '';
	if($section == 'photos' || $section == 'videos'){
		$divCustomStyle = 'style="background:'.$sm['theme']['search_card_bg']['val'].';cursor:pointer;width:275px!important;height:380px!important;"';
		$photoCustomStyle = 'style="height:380px!important;"';
		$removeFooter = 'display:none!important;';
	}

	if($section == 'photos'){
		$sm['search'][0]['profile_photo'] = getData('users_photos','photo','WHERE id = '.$custom);
	}


	if($sm['theme']['search_card_gradient']['val'] == 'Yes'){
		$search.= '<div class="search  '.$sm['theme']['search_card_bg']['val'].'" '.$divCustomStyle.'   data-search-user-card="'.$sm['search'][0]['id'].'" data-meet-user="meetLikeBtn'.$sm['search'][0]['id'].'" onclick="event.preventDefault();'.$goToProfile.'"><div class="photos-count" style="'.$removeFooter.'"><i class="mdi-image-camera-alt"></i><span>'.$sm['search'][0]['total_photos'].'</span></div>';
		$search.= '
		<div class="profile-photo  '.$blured.' vivify fadeIn delay-'.$delay.'" '.$photoCustomStyle.' onclick="event.preventDefault();'.$goToProfile.'" data-search-profile-link="'.$sm['search'][0]['id'].'" data-src="'.$sm['search'][0]['profile_photo'].'" ></div>';		
	} else {
		$search.= '<div class="search vivify fadeIn delay-'.$delay.'" '.$divCustomStyle.' data-search-user-card="'.$sm['search'][0]['id'].'" data-meet-user="meetLikeBtn'.$sm['search'][0]['id'].'" onclick="event.preventDefault();'.$goToProfile.'"><div class="photos-count" style="'.$removeFooter.'" ><i class="mdi-image-camera-alt"></i><span>'.$sm['search'][0]['total_photos'].'</span></div>';
		$search.= '
		<div class="profile-photo '.$blured.' vivify fadeIn delay-'.$delay.'" '.$photoCustomStyle.'onclick="event.preventDefault();'.$goToProfile.'" data-search-profile-link="'.$sm['search'][0]['id'].'" data-src="'.$sm['search'][0]['profile_photo'].'" ></div>';		
	}

	if($sm['search'][0]['last_access'] >= $time_now || $sm['search'][0]['fake'] == 1){
	...
	$search.= '<div class="footer" style="'.$removeFooter.$showGameBtn.'">'.$likedIcon;
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_object()){
			if($sm['user']['id'] != $row->id){
				...
				if($sm['search']['last_access'] >= $time_now || $sm['search']['fake'] == 1){
...
function meetFilterMobile($uid,$lang,$looking,$age,$radius,$status=0,$limi=0,$custom=''){
	global $mysqli,$sm;
	$search = '';
	$time_now = time()-300;
	$lat = $sm['user']['lat'];
...
function fbconnect($fuid,$name,$email,$gender,$location){
	...
	if ($check->num_rows == 1){	
		...
		$query = "INSERT INTO users (name,email,pass,age,gender,city,country,lat,lng,looking,lang,join_date,s_gender,s_age,verified,facebook_id,credits)
								VALUES ('".$name."', '".$email."','".crypt($pswd)."','20','".$gender."','".$city."','".$country."','".$lat."','".$lng."','".$looking."','".$_SESSION['lang']."','".$date."','".$looking."','18,60,1',1,'".$fuid."','".$sm['config']['free_credits']."')";	
...
function getTotalUsersCity($lat,$lng,$radius,$val2,$age,$city='') {
	...
	$stmt->bind_param("dddsiid", $lat, $lng, $lat, $gender, $age1, $age2, $radius);
	$stmt->execute();
...
function instaconnect($id,$name,$email,$gender,$photo,$location){
	...
	if ($check->num_rows == 1){	
		...
		$query = "INSERT INTO users (name,email,pass,age,gender,city,country,lat,lng,looking,lang,join_date,s_gender,s_age,verified,instagram_id,credits)
								VALUES ('".$name."', '".$email."','".crypt($pswd)."','20','".$gender."','".$city."','".$country."','".$lat."','".$lng."','".$looking."','".$_SESSION['lang']."','".$date."','".$looking."','18,60,1',1,'".$id."','".$sm['config']['free_credits']."')";	
		if ($mysqli->query($query) === TRUE) {
			$last_id = $mysqli->insert_id;
			$result = $last_id;
			$_SESSION['user'] = $last_id;	
...
function googleconnect($id,$name,$email,$gender,$photo,$location){
	...
	$country = isset($location->country_name)?$location->country_name:'N/A'; 	
	$lat = isset($location->latitude)?$location->latitude:'N/A'; 	
	$lng = isset($location->longitude)?$location->longitude:'N/A'; 	
...
$salt = base64_encode($name.$email);
		$query = "INSERT INTO users (name,email,pass,age,gender,city,country,lat,lng,looking,lang,join_date,bio,s_gender,s_age,credits,online_day,ip,last_access,username,join_date_time)
								VALUES ('".$name."', '".$email."','".crypt($pswd,$salt)."','23','".$gender."','".$city."','".$country."','".$lat."','".$lng."','".$looking."','".$lang."','".$date."','".$bio."','".$looking."','".$sage."',0,0,'".$ip."','".time()."','".$username."','".time()."')";					
		if ($mysqli->query($query) === TRUE) {
			$last_id = $mysqli->insert_id;
...
function twitterconnect($id,$name,$email,$gender,$photo,$location){
	...
	if ($check->num_rows == 1){	
		...
		$query = "INSERT INTO users (name,email,pass,age,gender,city,country,lat,lng,looking,lang,join_date,s_gender,s_age,verified,twitter_id,credits)
								VALUES ('".$name."', '".$email."','".crypt($pswd,$salt)."','20','".$gender."','".$city."','".$country."','".$lat."','".$lng."','".$looking."','".$_SESSION['lang']."','".$date."','".$looking."','18,60,1',1,'".$id."','".$sm['config']['free_credits']."')";	
		if ($mysqli->query($query) === TRUE) {
			$last_id = $mysqli->insert_id;
			$result = $last_id;
			$_SESSION['user'] = $last_id;	
...
//             ");
			...
			$stmt->bind_param("dddiii", $lat, $lng, $lat, $me, $gender, $diff);
			$stmt->execute();
			$spotlight = $stmt->get_result();
			if($spotlight && $spotlight->num_rows > 0){ 
				while($spotl = $spotlight->fetch_object()){	
...
function discoverStories($lat,$lng){
	...
	* cos( radians( lng ) - radians($lng) ) + sin( radians($lat) ) * sin(radians(lat)) ) ) AS distance 
	FROM users_story WHERE visible = 1 ORDER BY RAND() DESC LIMIT 0,20");
	if($query && $query->num_rows > 0){ 
		while($q = $query->fetch_object()){	
			...
			if (!in_array($id, $result)){
				...
				$storiesFilter = 'where uid = '.$id.' and storyTime >'.$storyFrom.' and deleted = 0 and visible = 1';
...
function discoverStoriesMobile($lat, $lng, $looking, $offset = 0) {
    global $sm, $mysqli;

    // Make sure session is started (for storing the random seed)
    if (session_status() === PHP_SESSION_NONE) {
        session_start();
    }

    // Cast inputs to be safe when interpolating into SQL
    $lat    = (float)$lat;
    $lng    = (float)$lng;
    $looking = (int)$looking;
    $offset  = (int)$offset;
    ...
    // Stories time window
    $storyFromDays = 365*3;//isset($sm['plugins']['story']['days']) ? (int)$sm['plugins']['story']['days'] : 1;
    $time = time();
    $extra = 86400 * $storyFromDays;
    $storyFrom = $time - $extra;
    $limit = 24;

    // Create / reset random seed only when offset = 0 (first "page")
    if ($offset === 0 || !isset($_SESSION['discover_stories_seed'])) {
        $_SESSION['discover_stories_seed'] = random_int(1, PHP_INT_MAX);
		$limit = 23;
    }
    $seed = (int)$_SESSION['discover_stories_seed'];

    $i = 0;
    
    $json = array();

    // Main query: optimized to fetch all needed data in single query
    // Gets latest story per user with all required fields to avoid N+1 queries
    $sql = "
        SELECT s1.id,
               s1.uid,
               s1.story,
               ...
               users.age,
               ( 6371 * acos(
                    cos( radians($lat) ) * cos( radians( s1.lat ) )
                    * cos( radians( s1.lng ) - radians($lng) ) 
                    + sin( radians($lat) ) * sin( radians(s1.lat) )
               )) AS distance 
        FROM users_story s1
        INNER JOIN (
            SELECT uid, MAX(storyTime) as maxTime
            FROM users_story
            WHERE visible = 1
              AND deleted = 0              
            GROUP BY uid
        ) s2 ON s1.uid = s2.uid AND s1.storyTime = s2.maxTime
        LEFT JOIN users ON users.id = s1.uid
        WHERE s1.visible = 1
          AND s1.deleted = 0
		  AND s1.storyTime >'.$storyFrom.'          
          AND users.gender = $looking
        ORDER BY RAND($seed)
        LIMIT $offset, $limit
    ";
    ...
    if ($query && $query->num_rows > 0) {
        while ($q = $query->fetch_object()) {
            ...
            $id = $q->uid;
                ...
                // All data already fetched in main query - no additional SQL needed!
                $story     = $q->story;
        ...
        // optional: expose seed in response if you ever want client-side control
        $json['seed'] = $seed;		
    } else {
        $json['result'] = 'empty';
    }
...
function getUserPhotosHeader($uid){	
	...
	if ($photos->num_rows > 0) { 
		while($up = $photos->fetch_object()){
			...
			if($up->blocked == 1){
				if($view == 0){
					$photo .= '<div class="relative profile-photo" data-src="'.$up->photo.'" style="display:inline-block;height:100%;cursor:pointer;" onClick="showAskPrivate()">
...
function unblockedUser($u1,$u2){
	...
	$u1 = secureEncode($u1);
	$u2 = secureEncode($u2);
	$premium = getData('users','premium','where id ='.$u1);
	$return = 0;
	...
	if($sm['premium']['private'] == 1 && $premium == 1){
		$return = 1;
	}
...
function getUserLC($uid){	
	...
	$time1 = 0;
	$time2 = 0;
	$query = $mysqli->query("SELECT s_id,id FROM chat WHERE r_id = '".$uid."' order by time DESC LIMIT 1");
	if ($query->num_rows > 0) { 
		while($result = $query->fetch_object()){
			$id1 = $result->s_id;
			$time1 = $result->id;
	...
	$query2 = $mysqli->query("SELECT r_id,id FROM chat WHERE s_id = '".$uid."' order by time DESC LIMIT 1");
	if ($query2->num_rows > 0) { 
		while($result2 = $query2->fetch_object()){
			$id2 = $result2->r_id;
			$time2 = $result2->id;
...
function getGuest($id){	
	...
	if ($query2->num_rows > 0) { 
		while($result2 = $query2->fetch_object()){
			$r = $result2->id;
			getUserInfo($id,1);
			$mysqli->query("UPDATE users SET lat = '". $sm['profile']['lat'] ."',lng = '". $sm['profile']['lng'] ."',city = '". $sm['profile']['city'] ."',country = '". $sm['profile']['country'] ."',lang = '".$sm['profile']['lang']."',s_age ='18,60,1',s_gender = '".$sm['profile']['gender']."' where id = '".$r."'");
		}	
	}
	return $r;
}
...
function getChat($uid1,$uid2){	
	...
	if ($spotlight->num_rows > 0) { 
		while($spotl = $spotlight->fetch_object()){
			...
			if (strpos($m , 'DATE-REQUEST-') !== false) {
				$dateData = explode('-',$m);
				$dateId = $dateData[2];
				$dateMsg = getData('dating','dating_description','WHERE id = '.$dateId);
				$content = '
                    <span class="message__fromStory" style="opacity:.6;font-size:10px;">'.$sm['lang'][979]['text'].'</span>
                    </span><br>				
                    <span class="message__fromStoryMessage">'.$sm['lang'][980]['text'].':<br><br>
                    <small>"'.$dateMsg.'"</small>';
			}
...
function deleteData($table,$filter=''){
	global $mysqli;
	$mysqli->query("DELETE FROM $table $filter");
}
...
function generateKey($keyLength=4) {
    $key = mt_rand(1000, 9999);
    return $key;
}

function sendSMS($provider,$number,$msg){
	global $mysqli,$sm;

	if($provider == 'lox24'){
		$uri = 'https://api.lox24.eu/sms';
		$token = $sm['plugins']['verification']['phone_carrier_key'];
		$body = [
		    'sender_id' => $sm['config']['name'],
		    'text' => $msg,
		    'service_code' => 'direct',
		    'phone' => '+'.$number,
		    'delivery_at' => time(),
...
function getvaluemetax($last,$server){
	global $sm;
	$first="";
	foreach ($sm['metasettings'] as $key => $value) {
    	if($sm['metasettings'][$key]==$server){
